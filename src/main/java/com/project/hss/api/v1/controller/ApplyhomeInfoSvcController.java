...
public class ApplyhomeInfoSvcController {
    ...
    @Operation(summary = "오피스텔/도시형/(공공지원)민간임대 분양정보 조회 (완)", description = "오피스텔/도시형/(공공지원)민간임대 분양정보 조회", tags = "ApplyhomeInfoSvc", security = {@SecurityRequirement(name = "Bearer")})
    @PostMapping("/getNotAPTLttotPblancList")
    public ResponseEntity<?> getNotAPTLttotPblancList(@Validated NotAPTLttotPblancListReq notAPTLttotPblancListReq, Errors errors) throws IOException {
        // validation check
        if (errors.hasErrors()) {
            return response.invalidFields(Helper.refineErrors(errors));
        }
    ...
    @Operation(summary = "APT무순위/취소후재공급 분양정보 조회 (완)", description = "APT무순위/취소후재공급 분양정보 조회", tags = "ApplyhomeInfoSvc", security = {@SecurityRequirement(name = "Bearer")})
    @PostMapping("/getRemndrLttotPblancList")
    public ResponseEntity<?> getRemndrLttotPblancList(@Validated RemndrLttotPblancListReq remndrLttotPblancListReq, Errors errors) throws IOException {
        // validation check
        if (errors.hasErrors()) {
            return response.invalidFields(Helper.refineErrors(errors));
        }
    ...
    @Operation(summary = "APT 분양정보 상세 조회 (완)", description = "APT 분양정보 상세 조회", tags = "ApplyhomeInfoSvc", security = {@SecurityRequirement(name = "Bearer")})
    ...
    @Operation(summary = "오피스텔/도시형/(공공지원)민간임대 분양정보 주택형별 상세 조회 (완)", description = "오피스텔/도시형/(공공지원)민간임대 분양정보 주택형별 상세 조회", tags = "ApplyhomeInfoSvc", security = {@SecurityRequirement(name = "Bearer")})
    @PostMapping("/getUrbtyOfctlLttotPblancMdl")
    public ResponseEntity<?> getUrbtyOfctlLttotPblancMdl(@Validated UrbtyOfctlLttotPblancMdlReq urbtyOfctlLttotPblancMdlReq, Errors errors) throws IOException {
        // validation check
        if (errors.hasErrors()) {
            return response.invalidFields(Helper.refineErrors(errors));
        }
    ...
    @Operation(summary = "APT무순위/취소후재공급 분양정보 주택형별 상세 조회 (완)", description = "APT무순위/취소후재공급 분양정보 주택형별 상세 조회", tags = "ApplyhomeInfoSvc", security = {@SecurityRequirement(name = "Bearer")})
    @PostMapping("/getRemndrLttotPblancMdl")
    public ResponseEntity<?> getRemndrLttotPblancMdl(@Validated RemndrLttotPblancMdlReq remndrLttotPblancMdlReq, Errors errors) throws IOException {
        // validation check
        if (errors.hasErrors()) {
            return response.invalidFields(Helper.refineErrors(errors));
        }
